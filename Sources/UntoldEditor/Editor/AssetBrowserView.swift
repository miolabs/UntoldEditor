            ...
            let items = contents.compactMap { item -> Asset? in
                ...
                if FileManager.default.fileExists(atPath: item.path, isDirectory: &isDir) {
                    ...
                    if isDir.boolValue {
                        ...
                        let allowedExtensions: Set<String> = Set([runtimeAssetExtension, "utex", "png", "jpg", "jpeg", "hdr", "exr", "tif", "tiff", "ply", "untoldgs", "json", "uscript", "remotestream"]).union(sourceAssetExtensions)
